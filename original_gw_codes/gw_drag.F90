...
subroutine gw_init()
  ...
  if (use_gw_rdg_beta) then
     ...
     call addfld('UEGW',  (/ 'lev' /) , 'I'  ,'s-1' ,  &
          'Zonal wind profile-entry to GW ' )
     call addfld('VEGW',  (/ 'lev' /) , 'I'  ,'s-1' ,  &
          'Merdional wind profile-entry to GW ' )
     call register_vector_field('UEGW','VEGW')
     call addfld('TEGW',  (/ 'lev' /) , 'I'  ,'K' ,  &
          'Temperature profile-entry to GW ' )
     call addfld('QEGW',  (/ 'lev' /) , 'I'  ,'kg kg-1' ,  &
          'Moisture profile-entry to GW ' )
     call addfld('ZIEGW',  (/ 'ilev' /) , 'I'  ,'m' ,  &
          'interface geopotential heights in GW code ' )
     call addfld('ZMEGW',  (/ 'lev' /) , 'I'  ,'m' ,  &
     ...
     call addfld('PINTEGW',  (/ 'ilev' /) , 'I'  ,'Pa' ,  &
          'interface pressure in GW code ' )
     call addfld('PILNEGW',  (/ 'ilev' /) , 'I'  ,'Pa' ,  &
          'log of interface pressure in GW code ' )
     call addfld('PSEGW',  horiz_only  , 'I'  ,'Pa' ,  &
          'surface press in GW code ' )

     call addfld('TAUM1_DIAG' , (/ 'ilev' /) , 'I'  ,'N m-2' , &
          'Ridge based momentum flux profile')
     call addfld('TAUM1_DIAG_A' , (/ 'ilev' /) , 'I'  ,'N m-2' , &
          'Ridge based momentum flux profile')
     call addfld('TAUM1_DIAG_B' , (/ 'ilev' /) , 'I'  ,'N m-2' , &
          'Ridge based momentum flux profile')
     call addfld('TAUM1_DIAG_C' , (/ 'ilev' /) , 'I'  ,'N m-2' , &
          'Ridge based momentum flux profile')
  ...
  if (use_gw_movmtn_pbl) then
     ...
     call addfld ('VORT4GW', (/ 'lev' /), 'A', '1/S', &
          'Vorticity')
     call addfld ('XPWP_MOVMTN', (/ 'lev' /), 'A', 'na', &
          'PBL Source')
  end if
...
subroutine gw_tend(state, pbuf, dt, ptend, cam_in, flx_heat)
  ...
  if (use_gw_movmtn_pbl) then
     ...
     call outfld ('XPWP_MOVMTN', xpwp_clubb, pcols, lchnk)
  ...
  if (use_gw_rdg_beta) then
     ...
     call outfld('QEGW', q(:,:,1) ,  ncol, lchnk)
     call outfld('ZIEGW', zi , ncol, lchnk)
     call outfld('ZMEGW', zm , ncol, lchnk)
     !++jtb - completing
     call outfld('PSEGW', ps ,  ncol, lchnk)
     call outfld('PINTEGW', pint ,  ncol, lchnk)
     call outfld('PILNEGW', piln ,  ncol, lchnk)
     call outfld('RHOIEGW', rhoi ,  ncol, lchnk)
     call outfld('NIEGW', ni ,  ncol, lchnk)
     call outfld('NMEGW', nm ,  ncol, lchnk)
...
subroutine gw_rdg_calc( &
   ...
   do nn = 1, n_rdg
      ...
      if (nn == 1) then
         call outfld('TAUM1_DIAG_A' , tau(:,0,:) ,  ncol, lchnk)
      end if
      ...
      if (nn == 1) then
         call outfld('TAUM1_DIAG_B' , tau(:,0,:) ,  ncol, lchnk)
      end if
