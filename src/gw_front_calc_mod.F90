...
subroutine gw_front_calc( &
   ...
   u, v, t, frontgf, &
   p, piln, zm, zi, &
   nm, ni, rhoi, kvtt, q, dse, effgw_cm, &
   ...
   real(r8),         intent(in) :: effgw_cm        ! Tendency efficiency.
   ...
   call alloc_err(istat,'gw_front_calc','tau',ncol*(band_front%ngwv**2+1)*(pver+1))
   allocate(gwut(ncol,pver,-band_front%ngwv:band_front%ngwv),stat=istat)
   call alloc_err(istat,'front_calc','gwut',ncol*pver*(band_front%ngwv**2+1))
   allocate(phase_speeds(ncol,-band_front%ngwv:band_front%ngwv),stat=istat)
   call alloc_err(istat,'front_calc','phase_speeds',ncol*(band_front%ngwv**2+1))
   ...
   write(*,*) " in front_calc shape( tau ) ",shape( tau )
   write(*,*) "   ... ngwv ", band_front%ngwv
   ...
   effgw(:) = effgw_cm
   write(*,*) " In gw_front_calc after effgw(:)= "
   ...
   write(*,*) " in gw_front_calc after gw_cm_src "
   ...
   write(*,*) " wheeeewww   ... survived gw_front_calc"

   ! Tau in each direction.
   call ncfile_put_col3d('TAUE', taucd(:,:,east)  , itime, 'N m-2', 'E stress profile - frontal' )
   call ncfile_put_col3d('TAUW', taucd(:,:,west)  , itime, 'N m-2', 'W stress profile - frontal' )
   call ncfile_put_col3d('TAUN', taucd(:,:,north)  , itime, 'N m-2', 'N stress profile - frontal' )
   call ncfile_put_col3d('TAUS', taucd(:,:,south)  , itime, 'N m-2', 'S stress profile - frontal' )
...
write(*,*) " Band Front ",band_front%ngwv
